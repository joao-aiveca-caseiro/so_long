...
/*   Updated: 2023/03/17 07:06:34 by jaiveca-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_win	move_enemies(t_win window)
{
	...
	while (window.map[++y])
	{
		...
		{
			n = rand() % 4;
			window = move_enemies_left_right(window, x, y, n);
			window = move_enemies_up_down(window, x, y, n);
		}
		x = -1;
	}
...
{
	if (window.map[y][x] == 'N' && (window.map[y][x + 1] == '0'
		|| window.map[y][x + 1] == 'P') && n == 0)
	{
	...
	else if (window.map[y][x] == 'N' && (window.map[y][x - 1] == '0'
		|| window.map[y][x - 1] == 'P') && n == 1)
	{
		render_image(window, window.floor, x * 32, y * 32);
		render_image(window, window.enemy, x * 32 - 32, y * 32);
		if (window.map[y][x - 1] == 'P')
...
{
	if (window.map[y][x] == 'N' && (window.map[y + 1][x] == '0'
		|| window.map[y + 1][x] == 'P') && n == 2)
	{
		render_image(window, window.floor, x * 32, y * 32);
	...
	else if (window.map[y][x] == 'N' && (window.map[y - 1][x] == '0'
		|| window.map[y - 1][x] == 'P') && n == 3)
	{
		render_image(window, window.floor, x * 32, y * 32);
		render_image(window, window.enemy, x * 32, y * 32 - 32);
		if (window.map[y - 1][x] == 'P')
